...
type PriceManagementModalProps = {
  ...
  onSaveSuccess: (updatedVariants: Array<{ productId: string | number; variant: ProductVariantUI }>) => void;
};
...
export function PriceManagementModal({
  ...
  const calcNetPrice = (sellPrice: number | null | undefined): number | null => {
    if (sellPrice === null || sellPrice === undefined || sellPrice <= 0) return null;
  ...
  const getVariantData = useCallback(
    (variant: ProductVariantUI, fallbackProduct: ProductUI) => {
      ...
      const unitCostUAH = (priceCNY * (rateCNY > 0 ? rateCNY : 1)) + shippingUA + managementUAH;
      const actualNetPrice = netPrice ?? calcNetPrice(priceInUA) ?? 0;
      const unitMarginUAH = actualNetPrice > 0 ? actualNetPrice - unitCostUAH : 0;
  ...
  const handleUpdateDraft = (
    ...
    const origPurchasedCount = variant.purchasedCount ?? fallbackProduct.totalPurchased ?? 0;
    const origSellsCount = variant.sellsCount ?? fallbackProduct.sellsCount ?? 0;
  ...
  const filteredProducts = useMemo(() => {
    return products.filter((p) => {
      ...
      // Only changed filter
      if (onlyChanged) {
        const hasDirty = (p.variantsList && p.variantsList.length > 0 ? p.variantsList : [{ id: undefined, priceCNY: p.priceCNY }]).some((v) => {
  ...
  const globalModalStats = useMemo(() => {
    ...
    products.forEach((p) => {
      const variants = p.variantsList && p.variantsList.length > 0 ? p.variantsList : [{ id: undefined, priceCNY: p.priceCNY }];
      variants.forEach((v) => {
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      for (const p of products) {
        const variants = p.variantsList && p.variantsList.length > 0 ? p.variantsList : [];
        for (const v of variants) {
          ...
          if (draft) {
            updatesPayload.push({
              ...
              priceInUA: draft.priceInUA !== null ? Number(draft.priceInUA) : null,
  ...
  return (
    ...
    >
      <div
        className="w-full max-h-[92vh] sm:max-h-[88vh] sm:max-w-4xl bg-background/95 border-t sm:border border-white/15 rounded-t-[32px] sm:rounded-[28px] shadow-2xl flex flex-col overflow-hidden backdrop-blur-2xl transition-transform duration-150 animate-in slide-in-from-bottom duration-300"
        style={{
      ...
      >
        ...
        <div className="px-4 sm:px-6 py-3.5 border-b border-border/40 flex flex-col gap-3">
          ...
          <div className="grid grid-cols-3 gap-2 p-2.5 rounded-2xl bg-foreground/[0.03] border border-foreground/5 text-center">
            <div className="space-y-0.5">
              ...
              <p className="text-xs sm:text-sm font-black text-foreground">
                {globalModalStats.totalInvested.toLocaleString("ru-RU", { maximumFractionDigits: 0 })}{" "}
            ...
            <div className="space-y-0.5 border-x border-foreground/10 px-1">
              ...
              <p className="text-xs sm:text-sm font-black text-foreground">
                {globalModalStats.totalProjectedRev.toLocaleString("ru-RU", { maximumFractionDigits: 0 })}{" "}
                <span className="text-[10px] opacity-70">₴</span>
              </p>
            </div>
            ...
            <div className="space-y-0.5">
              ...
              <p
                className={`text-xs sm:text-sm font-black ${
                  globalModalStats.totalProjectedMargin >= 0 ? "text-primary" : "text-destructive"
                }`}
              >
                {globalModalStats.totalProjectedMargin >= 0 ? "+" : ""}
                {globalModalStats.totalProjectedMargin.toLocaleString("ru-RU", { maximumFractionDigits: 0 })}{" "}
                <span className="text-[10px] opacity-70">₴</span>
              </p>
            </div>
          </div>
          ...
          {filteredProducts.length === 0 ? (
            ...
            filteredProducts.map((product) => {
              ...
              return (
                <div
                  key={product.id}
                  className="p-3.5 sm:p-4 rounded-2xl bg-foreground/[0.02] hover:bg-foreground/[0.04] border border-foreground/[0.08] transition-all space-y-3"
                >
                  {/* Product Header Row */}
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="relative w-10 h-10 sm:w-11 sm:h-11 rounded-xl overflow-hidden bg-muted/30 shrink-0 border border-foreground/10">
                        <Image
                          src={product.img || "https://placehold.co/100x100?text=Item"}
                          alt={product.name}
                          fill
                          sizes="44px"
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-2 text-[10px] text-muted-foreground font-semibold">
                          <span>{variants.length} {variants.length === 1 ? "вариант" : "варианта"}</span>
                          {product.archive ? (
                            <span className="text-amber-500 font-bold">• Архив</span>
                          ) : null}
                        </div>
                      </div>
                    </div>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-2">
                              {variants.length > 1 && (
                                <span className="text-[10px] font-bold px-2 py-0.5 rounded-md bg-foreground/5 text-muted-foreground flex items-center gap-1">
                                  ...
                                  Вариант {vIdx + 1}
                                  {variant.pddSearchQuery && ` (${variant.pddSearchQuery})`}
                                </span>
                              )}
                              {data.isDirty && (
                            ...
                            {data.isDirty && (
                              <button
                                onClick={() => handleRevertVariant(data.variantKey)}
                            ...
                            <div className="space-y-1">
                              ...
                              <div className="relative">
                                ...
                                <input
                                  ...
                                  min="0"
                                  placeholder="0"
                                  value={data.priceInUA !== null ? data.priceInUA : ""}
                                  onChange={(e) =>
                                    handleUpdateDraft(
                                      variant,
                                      product,
                          ...
                          <div className="flex flex-wrap items-center justify-between gap-2 mt-2.5 pt-2 border-t border-foreground/5 text-[10px] font-bold">
                            <div className="flex items-center gap-2">
                              <span className="text-muted-foreground">Маржа с 1 шт:</span>
                              <span
                            ...
                            <div className="flex items-center gap-2">
                              <span className="text-muted-foreground">Прогноз прибыли:</span>
                              <span
                                className={
          ...
          <div className="text-xs text-muted-foreground">
            {globalModalStats.changedVariantsCount > 0 ? (
              <span className="font-bold text-amber-500">
                {globalModalStats.changedVariantsCount} {globalModalStats.changedVariantsCount === 1 ? "изменение" : "изменений"} не сохранено
              </span>
            ) : (
