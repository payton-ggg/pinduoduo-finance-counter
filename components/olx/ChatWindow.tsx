...
export function ChatWindow({
  ...
  if (!thread) {
    return (
      <div className="flex flex-col items-center justify-center h-full bg-card/60 rounded-2xl border border-border/40 p-8 text-center backdrop-blur-md shadow-lg">
        ...
        <h3 className="text-lg font-bold text-foreground">Выберите диалог для общения</h3>
        <p className="text-xs text-muted-foreground max-w-sm mt-1">
          Выберите переписку из списка слева, чтобы отвечать покупателям OLX, генерировать AI ответы и оформлять заказы
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full bg-card/60 rounded-2xl border border-border/40 overflow-hidden shadow-lg backdrop-blur-md relative">
      ...
      <div className="p-3.5 px-4 border-b border-border/30 flex items-center justify-between gap-3 bg-card/80">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              ...
              {thread.account && (
                <Badge variant="outline" className="text-[10px] py-0 px-1.5 bg-foreground/5 border-border/30">
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          >
            <Star
              className={`w-4 h-4 ${
                thread.isFavorite ? "text-amber-400 fill-amber-400" : "text-muted-foreground"
              }`}
            />
          </Button>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {loadingMessages ? (
          ...
          <div className="text-center py-12 text-muted-foreground space-y-1">
            ...
            <p className="text-[11px] opacity-70">Напишите первое сообщение покупателю ниже</p>
          </div>
        ) : (
      ...
      <div className="p-2 border-t border-border/20 bg-background/50 flex items-center justify-between gap-2 overflow-x-auto">
        <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar py-0.5">
          ...
          <Button
            ...
            className="h-8 px-2.5 rounded-xl bg-gradient-to-r from-violet-600/15 to-indigo-600/15 border-indigo-500/30 hover:border-indigo-500/60 text-indigo-400 hover:text-indigo-300 text-xs font-bold gap-1.5 shrink-0 transition-all shadow-sm"
          ...
          {quickTemplates.slice(0, 3).map((tpl) => (
            <button
              ...
              className="h-8 px-3 rounded-xl bg-foreground/5 hover:bg-foreground/10 text-muted-foreground hover:text-foreground text-[11px] font-semibold truncate max-w-[140px] transition-all border border-border/20 shrink-0"
            >
              {tpl.title}
            </button>
          ))}
      ...
      <form onSubmit={handleSend} className="p-3 bg-card/90 border-t border-border/30 flex items-end gap-2">
        <div className="flex-1 relative">
          <Textarea
            ...
            rows={2}
            className="w-full bg-foreground/5 border-none resize-none text-xs sm:text-sm rounded-xl py-2 pl-3 pr-3 focus:ring-1 focus:ring-primary/30 min-h-[44px] max-h-[120px]"
          />
        </div>
