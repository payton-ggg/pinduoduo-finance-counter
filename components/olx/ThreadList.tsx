					...
					<div className="text-center py-16 text-muted-foreground space-y-2">
						...
						<p className="text-[11px] opacity-70">
							{searchQuery
								...
								: "Новые сообщения от покупателей появятся здесь"}
						</p>
					</div>
				) : (
					threads.map((thread) => {
						...
						return (
							...
							>
								<div className="flex items-start justify-between gap-2 mb-1">
									<div className="flex items-center gap-2 min-w-0">
										...
										<div className="min-w-0">
											<div className="flex items-center gap-1.5">
												<span
													className={`text-xs truncate ${hasUnread ? "font-black text-foreground" : "font-semibold text-foreground/90"}`}
												>
								...
								<p
									className={`text-xs line-clamp-1 mt-0.5 ${hasUnread ? "text-foreground font-semibold" : "text-muted-foreground"}`}
								>
								...
								<button
									type="button"
									onClick={(e) =>
										onToggleFavorite(thread.id, thread.isFavorite, e)
									}
									...
									title={
										thread.isFavorite ? "Убрать из избранного" : "В избранное"
									}
								>
									<Star
