...
const PRIVATE_URL = "https://api.intelligence.io.net/v1/private";
...
export async function POST(req: Request) {
  try {
    ...
    if (!IO_API_KEY) {
      return NextResponse.json({ error: 'IO_NET is not configured in .env' }, { status: 500 });
    }
    ...
    const attestationModels = modelsData.data.filter((m: any) => m.supports_attestation);
    const model = attestationModels.length > 0 ? attestationModels[0] : modelsData.data[0];
    
    const selectedModel = attestationModels.find((m: any) => m.model_id.toLowerCase().includes("llama-3")) || model;
    ...
    if (!attestationRes.ok) {
      throw new Error(`Failed to get attestation: ${attestationRes.statusText}`);
    }
...
If a field is not mentioned, omit it or set it to null/false/0 appropriately. Return only the JSON object, no markdown code blocks, no other text.`;
    ...
    if (signatureHeaders.signing_address.toLowerCase() !== signingAddress.toLowerCase()) {
      throw new Error("Signing address mismatch!");
    }

    const recoveredAddress = ethers.verifyMessage(signatureHeaders.text, signatureHeaders.signature);
    ...
    // Since DNS fails for api.intelligence.io.net, let's provide a mock fallback for testing the form!
    if (error.message.includes('fetch') || error.code === 'ENOTFOUND' || error.cause?.code === 'ENOTFOUND' || error.message.includes('getaddrinfo')) {
       return NextResponse.json({ 
           warning: "API fetch failed (DNS error). Returning mock data for demonstration.",
           data: {
               name: "AirPods Pro 2",
               priceCNY: 150,
               chip: "Airoha 1562AE",
